...
There is no public API endpoint that upgrades a member to `ADMIN`.
If an administrator account is needed for local demo or testing, it must be
provisioned directly in `auth_db.auth_users`.
...
- OAuth2 token and session data are delivered back through the URL fragment
